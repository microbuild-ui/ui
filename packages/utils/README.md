...
Utility functions for Buildpad UI packages. Includes field interface mapping, value formatting, and validation helpers.

## Installation

```bash
pnpm add @buildpad/utils
```
...
```tsx
import { isFieldReadOnly, isUUIDAutoGenerated, getAutoGenerationType } from '@buildpad/utils';
...
```tsx
import { isPresentationField } from '@buildpad/utils';
