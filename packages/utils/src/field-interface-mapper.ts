 ...
 * Supports explicit meta.interface from directus_fields table
 * 
 ...
 * Based on Directus field type mapping logic
 */
export function getFieldInterface(field: Field): InterfaceConfig {
  ...
  // Priority 1: Check for explicit interface in meta (from directus_fields table)
  if (meta?.interface) {
 ...
 * Maps Directus interface IDs to our component types
 */
    ...
    case 'list-m2m':
      return {
        ...
        props: {
          ...
          filter: options?.filter as Record<string, unknown>,
          // M2M-specific options from Directus
 ...
 * Based on Directus v-form setPrimaryKeyReadonly logic:
...
export function isFieldReadOnly(
  ...
  // Primary key behavior based on Directus setPrimaryKeyReadonly logic:
 ...
 * Based on Directus behavior where UUID primary keys are typically auto-generated
 */
export function isUUIDAutoGenerated(field: Field): boolean {
  ...
  // UUID primary keys are typically auto-generated (Directus pattern)
