...
import { useRelationM2O, useRelationM2OItem, type M2OItem } from "@microbuild/hooks";
...
export const ListM2O: React.FC<ListM2OProps> = ({
    ...
    const { relationInfo, loading: relationLoading, error: relationError } = useRelationM2O(collection, field);
    ...
    const loadAvailableItems = useCallback(async (searchTerm?: string) => {
        ...
        try {
            ...
            setAvailableItems(items as M2OItem[]);
        } catch (err) {
    ...
    }, [combobox.dropdownOpened, debouncedSearch, relationInfo, loadAvailableItems]);

    // Format display value for an item
    const formatDisplayValue = useCallback((item: M2OItem | null): string => {
        if (!item) return '';
    ...
    const handleSelect = useCallback((itemId: string | number | null) => {
        onChange?.(itemId);
        ...
        return (
            <Stack gap="xs">
                ...
                >
                    <Combobox.Target>
                        <InputBase
                            ...
                            rightSectionPointerEvents={value && allowNone && !disabled && !readOnly ? 'auto' : 'none'}
                            disabled={disabled || readOnly}
                ...
                <Modal
                    ...
                    title={`Create New ${relationInfo?.relatedCollection.collection || 'Item'}`}
                    size="lg"
                >
                    <Alert icon={<IconAlertCircle size={16} />} color="blue">
                        Create functionality will be available when CollectionForm component is implemented.
                    </Alert>
                </Modal>
            </Stack>
        );
                                        ...
                                        {itemsLoading ? (
                                            ...
                                            availableItems.map((item) => (
                                                <Table.Tr
                                                    ...
                                                    style={{
                                                        cursor: 'pointer',
                                                        backgroundColor: value === item.id ? 'var(--mantine-color-blue-light)' : undefined,
                                                    }}
                                                    onClick={() => {
